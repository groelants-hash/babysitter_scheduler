...
export default function App() {
    ...
    (async () => {
      try {
        const [sd, ud] = await Promise.all([
          window.storage.get(SLOTS_KEY, SHARED).catch(() => null),
          window.storage.get(USERS_KEY, SHARED).catch(() => null),
        ]);
        setSlotData(sd ? JSON.parse(sd.value) : defaultSlotData);
        setUsers(ud ? JSON.parse(ud.value) : defaultUsers);
      } catch {
  ...
  async function saveSlots(d) {
    ...
    try { await window.storage.set(SLOTS_KEY, JSON.stringify(d), SHARED); } catch {}
    setSaving(false);
  }

  async function saveUsers(u) {
    setUsers(u);
    try { await window.storage.set(USERS_KEY, JSON.stringify(u), SHARED); } catch {}
  }
